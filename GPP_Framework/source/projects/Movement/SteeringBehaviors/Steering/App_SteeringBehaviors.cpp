	...
	//UI
	{
		...
		for (UINT i = 0; i < m_AgentVec.size(); ++i)
		{
			...
			if (ImGui::CollapsingHeader(headerName))
			{
				...
				if (ImGui::Combo("", &a.SelectedBehavior, "Seek\0Context\0", 2))
				{
...
void App_SteeringBehaviors::SetAgentBehavior(ImGui_Agent& a)
{
	...
	case BehaviorTypes::Context:
		m_pContextSteering = new ContextSteering();
		m_pContextSteering->SetArraySize(a.nrOfArrows);
		for (Obstacle* obstacle : m_Obstacles)
		{
			m_pContextSteering->AddObstacle( obstacle );
		}
		a.pBehavior = m_pContextSteering;
		break;
...
void App_SteeringBehaviors::AddObstacle(ImGui_Agent& a)
{
	...
	if (positionFound)
	{
		Obstacle* pObstacle = new Obstacle(pos, radius);

		m_Obstacles.push_back(pObstacle);
		if (a.SelectedBehavior == int(BehaviorTypes::Context))
			m_pContextSteering->AddObstacle(pObstacle);
	}
